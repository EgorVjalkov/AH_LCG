...
def probability_of_list(result, chaos_bag_for_mutations, num=(0,'succeed'), skill_test=0):
    # print(result)
    mutated = list(map(lambda i: i + result, chaos_bag_for_mutations))
    # print(mutated)
    if 'fail' in num[1]:
        mutated = list(map(lambda i: -skill_test if i < -80 else i, mutated))
        if 'less' in num[1]:
            filter_func = lambda i: i < 0 and i >= num[0]
        else:
            filter_func = lambda i: i <= num[0] and i > -30
    else:
        filter_func = lambda i: i >= num[0]
    filtered = list(filter(filter_func, mutated))
    # print(filtered)
    return len(filtered)
